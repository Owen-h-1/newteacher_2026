  ...
  <div class="publish-task-page">
    ...
    <div class="form-card">
        ...
        <div class="question-editor">
          ...
          <div class="form-row">
            ...
            <button class="btn-add-question" @click="addQuestion">
              <i :class="editingQuestionIndex >= 0 ? 'fas fa-floppy-disk' : 'fas fa-plus'"></i>
              {{ editingQuestionIndex >= 0 ? "保存修改" : "添加到试卷" }}
            </button>
            <button v-if="editingQuestionIndex >= 0" class="btn-cancel-edit" @click="cancelEditQuestion">
              取消编辑
            </button>
          </div>
        </div>
      ...
      <div class="ai-quick-entry">
        ...
        <button class="btn-import-doc" :disabled="importingDoc" @click="triggerDocImport">
          <i class="fas fa-file-import"></i> {{ importingDoc ? "识别中..." : "文档导入题目" }}
        </button>
        <input
          ref="docInputRef"
          type="file"
        ...
        <div v-for="(q, index) in questionList" :key="index" class="question-item">
          ...
          <div class="question-main">
            <div class="question-header">
              ...
              <span class="ops-inline-always">
                操作：
                <a href="javascript:void(0)" @click.prevent="editQuestion(index)">编辑</a>
                /
                <a href="javascript:void(0)" @click.prevent="removeQuestion(index)">删除</a>
              </span>
            </div>
            ...
            <div class="question-inline-actions">
              <button class="btn-edit-text" @click="editQuestion(index)">编辑</button>
              <button class="btn-delete-text" @click="removeQuestion(index)">删除</button>
            </div>
          </div>
          <div class="question-ops desktop-only">
            <button class="btn-edit" @click="editQuestion(index)" title="编辑">
    ...
    <div v-if="previewVisible" class="preview-mask" @click.self="previewVisible = false">
      <div class="preview-dialog">
        <div class="preview-header">
          <h3>{{ formData.title || "未命名试卷" }}</h3>
          <button class="btn-close-preview" @click="previewVisible = false">
            <i class="fas fa-xmark"></i>
          </button>
        </div>
        <div class="preview-meta">
          <span>班级：{{ classOptions.find((c) => String(c.classId) === String(formData.classId))?.className || "--" }}</span>
          <span>截止：{{ formData.deadline || "--" }}</span>
          <span>总分：{{ totalScore }} 分</span>
        </div>
        <div class="preview-body">
          <div v-for="(q, idx) in questionList" :key="`preview_${idx}`" class="preview-question">
            <div class="preview-q-title">
              {{ idx + 1 }}. [{{ getTypeLabel(q.type) }}] {{ q.content }}
              <span class="preview-score">（{{ q.score || 0 }}分）</span>
            </div>
            <div v-if="q.options" class="preview-options" v-html="formatOptions(q.options)"></div>
            <div class="preview-answer">答案：{{ q.answer || "（未设置）" }}</div>
          </div>
        </div>
      </div>
    </div>
...
import {
  ...
  importTeacherHomeworkQuestions,
} from "@/utils/api";
...
const normalizeQuestionType = (type) => {
  ...
  if (["choice", "single", "select", "mcq", "选择题"].includes(t)) return "choice";
  if (["fill", "blank", "填空题"].includes(t)) return "fill";
  if (["essay", "qa", "subjective", "解答题", "问答题"].includes(t)) return "essay";
  return "essay";
};
...
const toEditableQuestion = (q) => ({
  ...
  score: Number(q?.score) > 0 ? Number(q.score) : 5,
});
...
const addQuestion = () => {
  ...
  const question = {
    type: normalizeQuestionType(currentQuestionType.value),
  ...
  if (editingQuestionIndex.value >= 0) {
    questionList.value.splice(editingQuestionIndex.value, 1, question);
    showToast("success", "题目已更新", "fa-check-circle");
  } else {
...
const editQuestion = (index) => {
  const q = toEditableQuestion(questionList.value[index]);
  if (!q) return;
  ...
  newQuestion.score = q.score;
  showToast("info", `正在编辑第 ${index + 1} 题`, "fa-pen");
};
...
const triggerDocImport = () => {
  if (importingDoc.value) return;
...
const onDocFileChange = async (e) => {
  ...
  try {
    const ret = await importTeacherHomeworkQuestions(file);
    const list = Array.isArray(ret?.list) ? ret.list : [];
    if (!list.length) {
      showToast("error", "未识别到题目，请检查文档格式", "fa-circle-exclamation");
      return;
    }
    questionList.value.push(
      ...list.map((q) => toEditableQuestion(q))
    );
    showToast("success", `已导入 ${list.length} 道题`, "fa-file-import");
  } catch (err) {
    showToast("error", err.message || "文档导入失败", "fa-circle-exclamation");
  } finally {
