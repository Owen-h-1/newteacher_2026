      ...
      <div class="card-header">
        <div class="header-left">
          ...
          <div class="class-stats">
            <span class="chip">作业数 {{ homeworks.length }}</span>
            <span class="chip chip-accent">平均完成率 {{ averageSubmitRate }}%</span>
          </div>
        </div>
        <div class="header-actions">
          <select v-model="selectedClassId" class="class-select" @change="handleClassChange">
            <option v-for="c in classOptions" :key="c.classId" :value="String(c.classId)">
              {{ c.className }}
            </option>
          </select>
...
const averageSubmitRate = computed(() => {
  if (!homeworks.value.length) return 0;
  const sum = homeworks.value.reduce((acc, x) => acc + Number(x.submitRate || 0), 0);
  return Math.round(sum / homeworks.value.length);
});

const submissionRowKey = (s) => `${s.studentNo}__${s.submitAccount || ""}`;

const loadHomeworks = async () => {
  const selectedId = String(selectedClassId.value || "").trim();
  if (!selectedId) {
    homeworks.value = [];
  ...
  loading.value = true;
  try {
    const data = await fetchTeacherHomeworkManageList(selectedId);
    const selected = classOptions.value.find((x) => String(x.classId) === selectedId);
    const selectedClassName = selected?.className || "";
    const raw = data.list || [];
    homeworks.value = raw.filter((hw) => {
      const cid = String(hw?.classId || "").trim();
      const cname = String(hw?.className || "").trim();
      if (cid) return cid === selectedId;
      return !!selectedClassName && cname === selectedClassName;
    });
    const existed = homeworks.value.find((x) => x.id === selectedHomeworkId.value);
    if (!existed) {
      selectedHomeworkId.value = "";
      submissions.value = [];
      detail.value = null;
...
loadClasses().then(loadHomeworks);
</script>

<style scoped>
.homework-manage-page { display: grid; gap: 14px; }
.page-header { display: flex; align-items: center; gap: 12px; }
.title-icon { width: 44px; height: 44px; border-radius: 12px; background:#1e6df2; color:#fff; display:flex; align-items:center; justify-content:center; }
...
.class-stats { display:flex; align-items:center; gap: 8px; }
.chip {
  display: inline-flex;
  align-items: center;
  height: 24px;
...
.class-select {
  ...
  background: #fff;
  color: #0b2b4a;
  box-shadow: 0 2px 8px rgba(30, 109, 242, 0.08);
}
